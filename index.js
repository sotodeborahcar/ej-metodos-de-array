...
const artistasPorSolista = (artistas) => {
  const artistaSolista = artistas.filter((artista) => {
    if (artista.solista) {
      return artista;
    }
  });
...
console.log("🐠 artistas Solistas", artistasPorSolista(artistas));
...
console.log("🐠 artistas Por Edad", artistasPorEdad(24, artistas));
...
const artistaConMasDiscosQue = () => {
  const artistasConMasDiscos = artistas.sort((a, b) => {
...
console.log("🐠 artistas Con Mas discos que", artistaConMasDiscosQue()); // lista del array de menor a mayor deacuerdo al numero de discos lanzados

const artistasConMasDiscosQue2 = (array, cantidadDeDiscos) => {
...
console.log(
  "🐠 artistas con mas discos 2",
  artistasConMasDiscosQue2(artistas, 5)
); // filtra el array por cantidad de discos mayores a 5 en orden
...
const artistaConMasEntradasVendidas = (array) => {
  const artistasConMasEntradas = array.reduce((acc, curr) => {
    //console.log("acc", acc.ultimoRecital.entradasVendidas);
    //console.log("curr", curr.ultimoRecital.entradasVendidas);
    if (
      acc.ultimoRecital.entradasVendidas > curr.ultimoRecital.entradasVendidas
    ) {
      return acc;
    } else {
      return curr;
  ...
  return artistasConMasEntradas;
};

console.log(
  "🐠 artistas Con Mas Entradas Vendidas",
...
const artistaConMayorRecaudacion = (array) => {
  const artistasConMasRecaudadas = array.reduce((acc, curr) => {
    if (
      acc.ultimoRecital.entradasVendidas * acc.ultimoRecital.costoEntradas >
      curr.ultimoRecital.entradasVendidas * curr.ultimoRecital.costoEntradas
    ) {
      return acc;
    } else {
      return curr;
  ...
  return artistasConMasRecaudadas;
};

console.log(
  "🐠 artista con mayor Recaudacion",
...
const artistaConDiscosEnAnio = (array, anio) => {
  const artistaConDiscosEn2000 = array.filter((artista) =>
    artista.discos.some((artista) => artista.anioLanzamiento === anio)
  );
  return artistaConDiscosEn2000;
};

console.log(
  "🐠 artista Con Discos En Año",
  artistaConDiscosEnAnio(artistas, 2000)
);
...
const cantidadDeArtistasPorGenero = (array) => {
  const cantidad = array.reduce((acc, curr) => {
    if (!acc[curr.genero]) {
      console.log("=>", curr.genero); // curr.genero => me da las propiedades del objeto {reggae, pop, grunge, cumbia, rock, punk, heavy}
      acc[curr.genero] = 1;
    } else {
      acc[curr.genero] = acc[curr.genero] + 1;
    }
    return acc;
  }, {});
  return cantidad;
...
console.log(
  "🐠 cantidad De Artistas Por Genero",
